...
func runInserter(conn pgxConn, in chan copyRequest) {
	...
	for {
		...
		//clear out the tail
		newLen := dst + 1
		for i := newLen; i < len(insertBatch); i++ {
			insertBatch[i] = copyRequest{}
		}
		insertBatch = insertBatch[:newLen]
		for i := 0; i < len(insertBatch); i++ {
			//for loop since may need to merge multiple table
			for i < len(insertBatch)-2 && insertBatch[i].table == insertBatch[i+1].table {
				insertBatch[i].data.absorb(insertBatch[i+1].data)
				insertBatch[i+1].data.release()
				//delete element i+1
				copy(insertBatch[i+1:], insertBatch[i+2:])
				insertBatch[len(insertBatch)-1] = copyRequest{}
				insertBatch = insertBatch[:len(insertBatch)-1]
			}
		}
